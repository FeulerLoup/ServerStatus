		...
		for (var i = 0; i < result.servers.length; i++) {
			...
			if (!result.servers[i].online4 && !result.servers[i].online6) {
				...
				if(result.servers[i].network_rx < 1000)
					netstr += result.servers[i].network_rx.toFixed(0) + "B/s";
				else if(result.servers[i].network_rx < 1000*1000)
					netstr += (result.servers[i].network_rx/1000).toFixed(0) + "K/s";
				else
					netstr += (result.servers[i].network_rx/1000/1000).toFixed(1) + "M/s";
				netstr += " | "
				if(result.servers[i].network_tx < 1000)
					netstr += result.servers[i].network_tx.toFixed(0) + "B/s";
				else if(result.servers[i].network_tx < 1000*1000)
					netstr += (result.servers[i].network_tx/1000).toFixed(0) + "K/s";
				else
					netstr += (result.servers[i].network_tx/1000/1000).toFixed(1) + "M/s";
